...
public class CsvQueryProcessor implements QueryProcessingEngine {
	...
	public DataSet getResultSet(QueryParameter queryParameter) {
		 ...
		 */
		try {
			br = new BufferedReader(new FileReader(queryParameter.getFile()));
			...
			while ((str = br.readLine()) != null) {
				...
				if (queryParameter.getRestrictions() != null) {
					...
					while (restrictionIter.hasNext()) {
						Iterator<Map.Entry<String, Integer>> headerIter = sortByValue(header.getHeaders()).entrySet()
								.iterator();
						Restriction rest = restrictionIter.next();
						while (headerIter.hasNext()) {
							Map.Entry<String, Integer> myEntry = headerIter.next();
